...
export default function Login() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      else if (msg.includes('Email not confirmed')) setError('Debes confirmar tu correo antes de ingresar. Revisa tu bandeja.')
      else setError(msg)
    } finally {
      setLoading(false)
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <button
          ...
          className="w-full bg-white text-sei-800 font-semibold text-sm py-2.5 rounded-xl
                     hover:bg-sei-50 transition-colors disabled:opacity-50"
        >
          {loading ? 'Ingresando...' : 'Ingresar'}
        </button>
      </form>

      {/* Enlace a registro para bomberos */}
      <div className="mt-5 pt-4 border-t border-white/10 text-center">
        <p className="text-white/50 text-xs">¿Eres bombero y no tienes cuenta?</p>
        <Link
          to="/registro"
          className="mt-1 inline-block text-white/80 text-xs font-medium underline hover:text-white transition-colors"
        >
