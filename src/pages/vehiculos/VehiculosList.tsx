...
const estadoStyle: Record<string, { bg: string; text: string; border: string; label: string }> = {
  [EstadoVehiculo.Operativo]:        { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/20', label: 'OPERATIVO' },
  [EstadoVehiculo.EnMantenimiento]:  { bg: 'bg-amber-500/10',   text: 'text-amber-400',   border: 'border-amber-500/20',   label: 'EN MTO.' },
  [EstadoVehiculo.FueraDeServicio]:  { bg: 'bg-red-500/10',     text: 'text-red-400',     border: 'border-red-500/20',     label: 'GROUNDED' },
  [EstadoVehiculo.Inspeccion]:       { bg: 'bg-blue-500/10',    text: 'text-blue-400',    border: 'border-blue-500/20',    label: 'INSPECC.' },
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from('estaciones')
        .select('id, nombre, codigo_iata, regional_id, regional:regionales(nombre, codigo)')
        .eq('activa', true)
        .order('nombre')
      return data ?? []
    },
...
export default function VehiculosList() {
  ...
  const { esNacional, esRegional, estacionId } = useScope()

  const [busqueda,          setBusqueda]          = useState('')
  const [filtroEstado,      setFiltroEstado]       = useState('todos')
  const [filtroEstacionSel, setFiltroEstacionSel]  = useState<string | null>(null)

  // Jefe Nacional puede filtrar por estación específica
    ...
    <div className="relative space-y-5">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <div className="relative flex-1 min-w-48">
          ...
          <input type="search" value={busqueda} onChange={e => setBusqueda(e.target.value)}
            ...
            className="w-full bg-slate-950 border border-white/5 rounded-xl pl-9 pr-4 py-2.5
                       ...
                       focus:outline-none focus:ring-1 focus:ring-blue-500/30 focus:border-white/10"/>
        </div>

        {/* Filtro por estación (solo nacional) */}
        {esNacional && (
        ...
        <div className="flex gap-1 bg-slate-950 border border-white/5 rounded-xl p-1">
          {[
            ...
            { v: EstadoVehiculo.Inspeccion,          l: 'INSPECCIÓN' },
          ].map(f => (
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {filtrados.map(v => {
            const es = estadoStyle[v.estado] ?? estadoStyle[EstadoVehiculo.Operativo]
            const estacion = v.estacion as { nombre?: string; codigo_iata?: string; ciudad?: string } | undefined
              ...
              <div key={v.id}
                ...
                <div className="flex items-start justify-between p-4 pb-3
                  ...
                  <span className={`text-[9px] font-bold px-2 py-1 rounded-lg border
                                   uppercase tracking-widest ${es.bg} ${es.text} ${es.border}`}>
                    {es.label}
                  </span>
                </div>
                ...
                <div className="p-4 space-y-2">
                  ...
                  {/* Estación — visible para jefe nacional */}
                  {esNacional && estacion && (
                    <div className="flex justify-between items-center pt-1 border-t border-white/5">
                      ...
                      <span className="text-[10px] font-mono font-bold text-blue-400">
                        {estacion.codigo_iata} — {estacion.nombre}
                      </span>
                    </div>
                  )}
                </div>
